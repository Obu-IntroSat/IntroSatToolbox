...
class CameraWidget(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.add_log("📌 Размер кадра и обрезка настраиваются")
    ...
    def _create_settings_panel(self) -> QWidget:
        ...
        layout.setSpacing(5)
        settings_group = QGroupBox("Настройки размера и обрезки")
        ...
        hint = QLabel("ℹ️ Сначала 'Применить' размер, затем 'Снимок'")
    ...
    def capture_image(self):
        if self.worker and self.state.connected:
            ...
            self.add_log(f"   Обрезка после загрузки: vStart={v_start}, hStart={h_start}")
    ...
    def apply_resolution(self):
        ...
        self.state.captured_width = w
        self.state.captured_height = h
        self.state.v_start = 0
        self.state.h_start = 0
        if self.worker and self.state.connected:
            self.worker.set_resolution(w, h)
            self.worker.set_crop_params(0, 0, w, h)
            self.v_start_spin.setValue(0)
            self.h_start_spin.setValue(0)
    ...
    def apply_crop(self):
        ...
        if self.worker and self.state.connected:
            self.worker.set_crop_params(v_start, h_start, w, h)
            self.add_log(f"✂️ Обрезка: vStart={v_start}, hStart={h_start}, {w}×{h}")
    ...
    def show_partial_image(self, data, w, h):
        """Отображение обрезанного изображения (постепенное заполнение)"""
        try:
            expected = w * h
            ...
            if progress == 100:
                ...
                self.add_log(f"✅ Изображение загружено: {w}×{h} (обрезано)")
                self._download_in_progress = False
